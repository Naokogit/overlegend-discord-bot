...
module.exports = {
    ...
    async execute(interaction) {
        if (
            ...
            interaction.customId === "btnClaimTicket"
        ) {
            if (!interaction.member.roles.cache.has(ticketsRole)) {
                await interaction.reply({
                    content: `❌ Non puoi effettuare questa azione`,
                    ephemeral: true,
                });
                return;
            }

            var channel = interaction.client.channels.cache.get(
            ...
            channel.messages.fetch({ limit: 100 }).then(async (messages) => {
                ...
                if (firstMessage) {
                    channel.messages
                        .fetch(firstMessage.id)
                        .then(async (message) => {
                            const row = new ActionRowBuilder();
                            console.log(message);
                            message.components[0].components.forEach(
                                (button) => {
                            ...
                            const embed = new EmbedBuilder()
                                ...
                                .setDescription(
                                    `Il ticket verrà gestito da <@${interaction.member.id}>`
                                )
                                .setTimestamp()
                            ...
                            const ticketInformation =
                                getTicketCacheInformation(interaction);
                            const query = {
                            ...
                            await ticket.updateOne(query, {
                                $set: { assignedTo: interaction.member.id },
                            });
